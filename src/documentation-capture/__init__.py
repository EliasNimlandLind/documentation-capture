...
base_directory = config['paths']['output_directory'].replace('%Y-%m-%d', time.strftime('%Y-%m-%d'))
...
arrow_length = int(config['arrow_properties']['arrow_length'])
arrow_width = int(config['arrow_properties']['arrow_width'])
...
def get_arrow_direction(point: tuple, screen_width, screen_height):
    ...
    angle = math.atan2(corner_y - y, corner_x - x)
...
def on_click(x, y, button, pressed):
    ...
    if pressed:
        ...
        angle, closest_corner = get_arrow_direction((x, y), screen_width, screen_height)
